...
###############################################################################
##### OPTIGRAB REPLACEMENT FUNCTIONS #####
# These functions replace the deprecated optigrab package functionality
# using base R's commandArgs()

# Store command line arguments globally for use by helper functions
.peppro_args <- commandArgs(trailingOnly = TRUE)

#' Get all command line arguments
#' @return Character vector of all arguments
opt_get_args <- function() {
    return(.peppro_args)
}

#' Get the subcommand (first non-option argument)
#' @return The subcommand string, or NA if not found
opt_get_verb <- function() {
    args <- opt_get_args()
...
#' Get option value(s) from command line arguments
#' @param name Character vector of option names (long and/or short)
#' @param required Logical, whether the option is required
#' @param default Default value if option not found
#' @param n Number of values to capture (default 1, use 0 for flags)
#' @param description Description of the option (unused, for compatibility)
#' @return The option value(s), or default if not found
opt_get <- function(name, required = FALSE, default = NULL, n = 1, 
                    description = "") {
    args <- opt_get_args()
    
    # Build list of option flags to search for
    flags <- character(0)
    for (nm in name) {
        if (nchar(nm) == 1) {
    ...
    if (!is.null(n) && !is.na(n) && n == 0) {
        ...
        return(if (is.null(default)) FALSE else default)
    }
    ...
    if (is.na(opt_pos)) {
        if (required) {
            stop(paste0("Required option not found: ", 
                        paste(flags, collapse = " or ")))
        }
        return(default)
    }
    
    # Get the value(s) following the option
    if (is.null(n) || is.na(n) || n == 1) {
        # Check if there's a next argument
        if (opt_pos + 1 > length(args)) {
            # No next argument - if default is boolean, treat as flag
            if (is.logical(default)) {
                return(TRUE)
            }
            if (required) {
                stop(paste0("Option ", args[opt_pos], " requires a value"))
            }
            return(default)
        }
        
        next_arg <- args[opt_pos + 1]
        
        # If next arg is another flag, treat this as a boolean flag
        if (grepl("^-", next_arg)) {
            if (is.logical(default)) {
                return(TRUE)
            }
        ...
        while (pos <= length(args) && count < n) {
            # Stop if we hit another option flag
            if (grepl("^-", args[pos])) {
                break
            }
            values <- c(values, args[pos])
            pos <- pos + 1
            count <- count + 1
        }
        ...
        if (length(values) == 0) {
            if (required) {
                stop(paste0("Option ", args[opt_pos], " requires value(s)"))
            }
            return(default)
        }
        return(values)
